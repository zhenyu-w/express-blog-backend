...
router.put("/login", function (req, res, next) {
  ...
  if (!req.session.captcha) {
    ...
    params.captcha.toLowerCase() != req.session.captcha.toLowerCase()
  ) {
    res.send({
      code: "001002",
      data: null,
    ...
    dbUtils.getConnection(res).then((connection) => {
      dbUtils
        .query(
          {
            sql: indexSQL.QueryByUserNameAndPwd,
            ...
            return dbUtils
              .query(
                {
                  ...
                  values: [
                    { last_login_time: new Date(), token: req.session.id },
                    results[0].id,
                  ],
                },
                connection,
              ...
              .then(() => {
                ...
                const expireTime = new Date(
                  Date.now() + 86400000 * config.cookieExpireDate
                );
